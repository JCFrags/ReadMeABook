...
4. **retry_missing_torrents** - Default: daily midnight, handles audiobook and ebook requests. Filters `nextSearchAt` (null or due) and release eligibility BEFORE the 50-request cap. Order: never-searched first, then oldest `lastSearchAt`, `createdAt`, and ID. Confirmed no-match delays: 1, 3, 7, then 14 days. Provider errors use separate retry eligibility without increasing the no-match count. Owns guarded `awaiting_search`/`awaiting_release` transitions under `indexer.skip_unreleased`. Enqueue rechecks state and eligibility, and reuses an existing live search. Enabled by default. See [search policy](../../phase3/search-policy.md).
